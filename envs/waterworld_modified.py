...
class Archea(Agent):
    ...
    def __init__(self, idx, radius, n_sensors, sensor_range, addid=True, speed_features=True, ally=1):
        self._idx = idx
        self._radius = radius
...
class MAWaterWorld_mod(AbstractMAEnv, EzPickle):
    ...
    def __init__(self, n_pursuers, n_evaders, n_coop=2, n_poison=10, radius=0.015,
        ...
        self._pursuers = [
            Archea(npu + 1, self.radius * 1., self.n_sensors, self.sensor_range[npu], addid=self._addid,
                   speed_features=self._speed_features, ally=self.n_pursuers) for npu in range(self.n_pursuers)
        ]
        self._evaders = [
            Archea(nev + 1, self.radius * 1., self.n_pursuers, self.sensor_range.mean() / 2)
            for nev in range(self.n_evaders)
        ]
        self._poisons = [
            Archea(npo + 1, self.radius * 1., self.n_poison, 0) for npo in range(self.n_poison)
        ]
    ...
    def step(self, action_Np2):
        ...
        time_of_catch, time_of_posion = 0, 0
        ...
        return obslist, rewards, done, [time_of_catch, time_of_posion]

    def render(self, screen_size=128, rate=10, mode='human', gui=False):
        import cv2
