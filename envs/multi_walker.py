...
LEG_FD = fixtureDef(shape=polygonShape(box=(LEG_W / 2, LEG_H / 2)), density=1.0,
...
class MultiWalkerEnv(AbstractMAEnv, EzPickle):
    ...
    def __init__(self, n_walkers=2, position_noise=1e-3, angle_noise=1e-3, reward_mech='local',
        ...
        EzPickle.__init__(self, n_walkers, position_noise, angle_noise, reward_mech, forward_reward,
                          fall_reward, drop_reward, terminate_on_fall, one_hot, **kwargs)
    ...
    def setup(self):
        ...
        # print('Local fixture creation.')
        self.fd_package_polygon = fixtureDef(
    ...
    def step(self, actions):
        ...
        # add pos shaping
        # pos_shaping = -abs(self.package.angle * 20)
        # rewards += (package_shaping - self.prev_package_shaping + pos_shaping)
        ...
        if self.reward_mech == 'local':
            return obs, rewards, done, {'pos': self.package.position.x}
        return obs, [rewards.mean()] * self.n_walkers, done, {'pos': self.package.position.x}
    ...
    def _generate_package(self):
        ...
        poly = [(x * self.package_scale / SCALE, y / SCALE)
                for x, y in PACKAGE_POLY]

        self.fd_package_polygon.shape.vertices = poly

        self.package = self.world.CreateDynamicBody(
            position=(init_x, init_y),
            fixtures=self.fd_package_polygon)
    ...
    def _generate_terrain(self, hardcore):
        ...
        for i in range(self.terrain_length):
            ...
            if state == GRASS and not oneshot:
                ...
                self.fd_polygon.shape.vertice = poly
                t = self.world.CreateStaticBody(fixtures=self.fd_polygon)
                t.color1, t.color2 = (1, 1, 1), (0.6, 0.6, 0.6)
                self.terrain.append(t)
                self.fd_polygon.shape.vertice = [(p[0] + TERRAIN_STEP * counter, p[1]) for p in poly]
                t = self.world.CreateStaticBody(fixtures=self.fd_polygon)
                t.color1, t.color2 = (1, 1, 1), (0.6, 0.6, 0.6)
                self.terrain.append(t)
                counter += 2
