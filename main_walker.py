...
with open('./config_walker.yaml', 'r') as f:
...
shutil.copy('./config_walker.yaml', result_folder + '/config_walker.yaml')
...
vis = visdom.Visdom(port=args.port, env='continuous-walker_{}_{}'.format(args.alg, unique_token),
        ...
        if ep_i % args.test_interval < args.n_rollout_threads:
            ...
            with th.no_grad():
                ...
                if pos_total > save_data['pos']:
                    ...
                    if record_r.sum() > 0 and pos_total > 10. and not has_saved:
                        # pathlib.Path(new_path).mkdir(parents=True, exist_ok=True)
                        maddpg.save(new_path)
                if pos_total > 17.0:
                    maddpg.save(new_path)
