...
class ReplayBuffer(object):
    ...
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims):
        """
        Inputs:
            max_steps (int): Maximum number of timepoints to store in buffer (size of buffer)
        ...
        for odim, adim in zip(obs_dims, ac_dims):
            ...
            self.rew_buffs.append(np.zeros(self.max_steps))  # TODO: the placeholder of reward is for each agent
    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        # norm rewards: normalize batch data
        inds = np.random.choice(np.arange(self.filled_i), size=N, replace=False)

        if to_gpu:
            cast = lambda x: th.cuda.FloatTensor(x)
        else:
            cast = lambda x: th.FloatTensor(x)
